...
import fr.kayrouge.tm.world.dimension.CorruptedDimensionDimension;
import fr.kayrouge.tm.itemgroup.MainTabItemGroup;
import fr.kayrouge.tm.TmModElements;

@TmModElements.ModElement.Tag
public class PetrolBlock extends TmModElements.ModElement {
	@ObjectHolder("tm:petrol")
	public static final FlowingFluidBlock block = null;
	@ObjectHolder("tm:petrol_bucket")
	public static final Item bucket = null;
	private FlowingFluid flowing = null;
	private FlowingFluid still = null;
	private ForgeFlowingFluid.Properties fluidproperties = null;
	public PetrolBlock(TmModElements instance) {
		super(instance, 10);
	...
	public void initElements() {
		fluidproperties = new ForgeFlowingFluid.Properties(() -> still, () -> flowing,
				FluidAttributes.builder(new ResourceLocation("tm:blocks/petrol_still"), new ResourceLocation("tm:blocks/petrol_flow")).luminosity(0)
						.density(1001).viscosity(996)).bucket(() -> bucket).block(() -> block);
		...
		elements.items.add(() -> new BucketItem(still, new Item.Properties().containerItem(Items.BUCKET).maxStackSize(1).group(MainTabItemGroup.tab))
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			biome.addFeature(GenerationStage.Decoration.LOCAL_MODIFICATIONS, new LakesFeature(BlockStateFeatureConfig::deserialize) {
				...
				public boolean place(IWorld world, ChunkGenerator generator, Random rand, BlockPos pos, BlockStateFeatureConfig config) {
					...
					if (dimensionType == CorruptedDimensionDimension.type)
