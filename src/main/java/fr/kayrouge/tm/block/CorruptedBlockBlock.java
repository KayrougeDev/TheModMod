...
public class CorruptedBlockBlock extends TmModElements.ModElement {
	@ObjectHolder("tm:corrupted_block")
	public static final Block block = null;
	public CorruptedBlockBlock(TmModElements instance) {
		super(instance, 8);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("corrupted_block", "corrupted_block", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 18)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(24, 4, 4, 77))));
		}
	}
}
