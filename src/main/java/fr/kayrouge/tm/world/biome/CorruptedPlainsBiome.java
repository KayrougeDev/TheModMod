...
package fr.kayrouge.tm.world.biome;
...
public class CorruptedPlainsBiome extends TmModElements.ModElement {
	@ObjectHolder("tm:corrupted_plains")
	public static final CustomBiome biome = null;
	public CorruptedPlainsBiome(TmModElements instance) {
		super(instance, 45);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.3f).depth(0.1f).scale(0.2f).temperature(0.7999999999999999f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.PLAINS).waterColor(-6750055).waterFogColor(-6750055).parent("plains")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.GRANITE.getDefaultState(), Blocks.GRANITE.getDefaultState())));
			setRegistryName("corrupted_plains");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.OAK_WOOD.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.BLACK_GLAZED_TERRACOTTA.getDefaultState()))).baseHeight(3)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || ground == Blocks.GRANITE.getDefaultState().getBlock())
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.BLACK_GLAZED_TERRACOTTA.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.BLACK_GLAZED_TERRACOTTA.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, Blocks.OAK_WOOD.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.OAK_WOOD.getDefaultState().getBlock()
					|| blockType == Blocks.BLACK_GLAZED_TERRACOTTA.getDefaultState().getBlock()
					|| blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || blockType == Blocks.GRANITE.getDefaultState().getBlock();
		}
